...
public class HomeController {
	...
	//Each URI template variable will have corresponding argument with @PathVariable annotation
	//Here template variable is same as argument variable name. 
	@RequestMapping(value = "/homePage/{month}/{year}")
	public String templateMappingOne(Model model, 
	...
	public String templateMappingTwo(Model model,
			@PathVariable("day") int dayVar) {
		model.addAttribute("message", " Day is " + dayVar);
		return "home";
	}
	/*
	 * The syntax for supporting regular expression is {variableName:regularExpression}
	 * e.g. We have to pass date in the yyyy-mm-dd format, to capture this we can define our mapping in the form of regular expression.
	 * If any invalid date is passed like 1525-18-15 then
	 * WARNING: No mapping found for HTTP request with URI [/springMVC/date/1525-18-16] in DispatcherServlet with name 'appServlet'
	 * will be thrown on server console. 
	 * 
	 */
	@RequestMapping(value = "/date/{year:[12][0-9]\\d\\d}-{month:0[1-9]|1[012]}-{day:0[1-9]|[12][0-9]|3[01]}")
	public String templateMappingWithRegularExpression(Model model,
	...
	/*
	 * The syntax for supporting a path pattern
	 * We can pass any thing in place of "*"
	 */
